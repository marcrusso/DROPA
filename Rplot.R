...
if (file.exists(NameInter2)) {
  ...
  a=paste('./', myArgs[1],'/',myArgs[1],'Intergenic.pdf', sep="")
  pdf(a)
...
frecParts=c(table(Upstream)[2],table(UTR5)[2],table(Exons)[2],table(Introns)[2],table(UTR3)[2],table(Downstream)[2])
  ...
  # If there is some part (Upstream, exons, etc) without peaks replace NA for 0
frecParts[is.na(frecParts)] <- 0
piepercent<- round(100*frecParts/sum(frecParts), 1)
NumbersInLegend=c(paste('Upstream:',table(Upstream)[2]),paste('5UTR:',table(UTR5)[2]),paste('Exon:',table(Exons)[2]),paste('Intron:',table(Introns)[2]),paste('3UTR:',table(UTR3)[2]),paste('Downstream:',table(Downstream)[2]))
nametitle=paste('Frequency of peaks in parts of the genes which are ','\n',gsub('_',' the FPKM limit',gsub(paste('FPKM',myArgs[1],sep=''),'',myArgs[2])))
adressfile=paste('./', myArgs[1],'/',myArgs[2],'_pie.pdf', sep="")
pdf(adressfile)
...
##### Pie charts of Intergenic and parts of the Genes. #####
